...
| 状態 | |
|---|---|
| ✅ 完了 | 憲章・研究プロトコル・文献の整備／選択バイアスの実証／リプレイと盲検装置の検証／テンプレート3種／研究契約の起草／**日本株データ源の確定（J-Quants・生存者バイアス無しを実測確認）**／スクリプトの共通化 |
| 🔜 次 | 契約の期間分割を見直し（J-Quants無料枠は**2年のみ**）→ 契約の凍結 → **フォワード記録の開始**（[#27](https://github.com/TIshow/tradingview-mcp/issues/27)） |
| ⏸ その後 | 判定装置（[#7](https://github.com/TIshow/tradingview-mcp/issues/7)）→ 20仮説で v0 を一周 |

**未解決**：J-Quants は約12週間の遅延があるため、**フォワード記録の当日シグナルには使えない**。別の当日データ源が必要（[#2](https://github.com/TIshow/tradingview-mcp/issues/2)）。
