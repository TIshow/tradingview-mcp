...
/** 決定的な乱数（mulberry32）。同じ種なら必ず同じ列を返す＝結果を再現できる。 */
function rng(seed) {
  let a = seed >>> 0;
  return () => {
    a = (a + 0x6D2B79F5) | 0;
...
/**
 * ランダム選択 — 判定の物差しとなる帰無仮説。
 *
 * ユニバースから毎週でたらめに20銘柄選ぶだけの「戦略」。
 * 20銘柄しか持たないポートフォリオの成績は、そもそも選択のばらつきに支配される。
 * ベースラインの数字がこのばらつきの中に埋もれるなら、その戦略に情報は無い。
 *
 * 種を変えて多数回まわし、分布と比べる（White のリアリティ・チェックの考え方。
 * docs/references.md 参照）。種を固定しているので何度実行しても同じ結果になる。
 */
export function randomPicker(seed) {
  const r = rng(seed);
  return {
    id: `random_${seed}`,
    name: `ランダム#${seed}`,
    rationale: '情報を持たない選択。ベースラインが超えるべき下限。',
    warmup: 252,
    rank: (panel, di, universe) => universe.map(si => ({ si, score: r() })),
  };
}
